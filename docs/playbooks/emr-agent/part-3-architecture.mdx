    ...
    **INSERT for new, UPDATE for returning**
    ...
    def _store_patient(self, data: Dict) -> int:
        ...
        if existing_id:
            ...
            # DatabaseMixin.update takes (table, data, where, params)
            self.update(
                "patients",
                data,
                "id = :id",
...
class CustomIntakeAgent(MedicalIntakeAgent):
    ...
    def _init_database(self):
        # Parent creates the base schema; the real method is _init_database
        super()._init_database()

        # Add custom columns (ALTER TABLE is idempotent-safe when wrapped)
        self.execute("""
