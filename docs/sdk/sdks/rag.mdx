...
# Or index multiple by looping
for path in ["guide1.pdf", "guide2.pdf", "guide3.pdf"]:
...
class ResearchAgent(Agent):
    ...
    def __init__(self, documents_path: str, **kwargs):
        self.rag = RAGSDK()
        # index_document takes one path; call in a loop for multiple files
        if isinstance(documents_path, (list, tuple)):
            for path in documents_path:
                self.rag.index_document(path)
        else:
            self.rag.index_document(documents_path)
...
rag = RAGSDK(config)
for path in ["faq.pdf", "troubleshooting.pdf", "quick_start.pdf"]:
    rag.index_document(path)
...
rag = RAGSDK(config)
for path in ["paper1.pdf", "paper2.pdf", "literature_review.pdf"]:
    rag.index_document(path)
...
rag = RAGSDK(config)
for path in ["api_reference.pdf", "tutorial.pdf", "examples.pdf"]:
    rag.index_document(path)
...
try:
    rag.index_document("/etc/passwd")  # Raises PermissionError
...
# 2. Index documents in a loop (each call processes one file)
for doc in ["doc1.pdf", "doc2.pdf", "doc3.pdf"]:
    rag.index_document(doc)

# 3. Tune chunk sizes and LLM-based chunking
config = RAGConfig(
    chunk_size=1024,          # Larger chunks = fewer embeddings = faster indexing
    use_llm_chunking=False,   # Disable semantic chunking for raw speed
)
...
# 3. Clear cached embeddings when switching projects
rag.clear_cache()
