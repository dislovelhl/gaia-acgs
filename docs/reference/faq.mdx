...
<AccordionGroup>
  <Accordion title="Where are my chats stored?" icon="database">
    Your chat database is at `~/.gaia/chat/chat.db` (a SQLite file). It is never sent off your machine. Documents you upload are at `~/.gaia/documents/`.
  ...
  <Accordion title="How do I uninstall GAIA?" icon="trash">
    ...
    By default, uninstalling keeps your chats and documents. To remove everything, run `~/.gaia/venv/bin/gaia uninstall --purge` from a terminal.
  </Accordion>
