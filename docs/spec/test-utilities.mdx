...
def assert_tool_result(result: Dict, expected_keys: List[str]) -> None:
    ...
    Usage:
        # Call the registered tool function directly — Agent has no public
        # `execute_tool()`; see gaia.agents.base.tools._TOOL_REGISTRY
        from gaia.agents.base.tools import _TOOL_REGISTRY
        result = _TOOL_REGISTRY["search"]["function"](query="test")
...
def test_customer_agent_creates_record(tmp_path):
    ...
    with temp_database(schema_file=str(schema)) as db_url:
        ...
        # Tools are regular Python functions — call them directly
        from gaia.agents.base.tools import _TOOL_REGISTRY
        result = _TOOL_REGISTRY["create_customer"]["function"](
            name="Test Customer"
        )
...
def test_form_extraction():
    ...
    # Test extraction — invoke the registered tool function directly
    from gaia.agents.base.tools import _TOOL_REGISTRY
    result = _TOOL_REGISTRY["extract_form"]["function"](
        image_path="fake.png"
    )
