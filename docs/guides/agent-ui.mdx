...
<Tabs>
  ...
  <Tab title="Python CLI">
    ...
    **Options:**
    ...
    | `gaia --ui --base-url http://192.168.1.100:13305/api/v1` | Connect to Lemonade on another machine (e.g., GPU workstation) |
...
Conversations initiated via MCP appear in the browser UI in real time, so you can watch tool execution and agent activity as it happens. When governance blocks a tool call, the same activity panel shows the policy alert inline and the notification center preserves the receipt details for later review.
