...
{
    "name": "get_weather",
    "description": "Get current weather for a city.\n\nUse this tool when ...",  # whole docstring
    "parameters": {
...
```python
{
    "status": "success",           # Required: "success", "error", or "partial"
    "message": "Brief summary",    # Recommended: Human-readable summary
    "data": {...},                 # When applicable: Structured data
    "instruction": "Guidance...",  # Recommended: How to use this data
}
```
...
def get_system_stats() -> dict:
    ...
    Use this tool when the user asks about system health,
    ...
    return {
        ...
        "data": {
            ...
            "memory_gb_used": stats.memory_used,
            "memory_gb_total": stats.memory_total,
            "disk_free_gb": stats.disk_free
        },
        "instruction": (
            "Summarize these statistics in a human-readable format. "
...
# Tools decorated with @tool are automatically registered
# and available to any Agent subclass via the global tool registry.
# No manual registration is needed in _register_tools().
```
...
def tool(func=None, *, atomic: bool = False, **kwargs):
    ...
    def decorator(f):
        ...
        for name, param in sig.parameters.items():
            ...
            # A small annotation → JSON-type map; unmapped types stay "unknown"
            ann = param.annotation
            if ann is str:   info["type"] = "string"
            elif ann is int: info["type"] = "integer"
            elif ann is float: info["type"] = "number"
            elif ann is bool:  info["type"] = "boolean"
            elif ann is tuple: info["type"] = "array"
            elif ann is dict:  info["type"] = "object"
            params[name] = info

        _TOOL_REGISTRY[f.__name__] = {
            "name": f.__name__,
            "description": f.__doc__ or "",  # whole docstring, not per-arg
            "parameters": params,
            "function": f,
            "atomic": atomic,
        }
...
- The full docstring is stored once as `description`. There's no per-argument
  docstring parsing today — the LLM sees the whole docstring together with a
  `{type, required}` map.
- Default values are **not** stored on the parameter entries. `required` is a
  boolean derived from whether the parameter has a default.
- Generic annotations like `list[str]` or `Optional[int]` are not specialised —
  only the bare primitives (`str`, `int`, `float`, `bool`, `tuple`, `dict`) map
